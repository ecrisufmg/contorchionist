...
torch::Tensor RFFTProcessor<T>::process_irfft(const std::vector<torch::Tensor>& input_spectrum_vec, long output_n,
    ...
    if (windowing_enabled_ && window_prepared_) {
        ...
        // --- INÍCIO DA CORREÇÃO ---
        // Calcula o ganho de compensação para o Overlap-Add
        // G = N / (overlap_factor * sum(window^2))
        if (overlap_factor_ > 0 && current_sum_sq_window_ > 1e-9) {
            T compensation_gain = output_n / (1 * current_sum_sq_window_);
            time_signal = time_signal/2.0f * compensation_gain; // nota=> compensação por 2 independe do overlap e funciona para hamming, hanning e blackman => provavelmente tem uma maneira mais elegante de formalizar isso!
        }
        // --- FIM DA CORREÇÃO ---
    }

    // A divisão por overlap_factor_ que você já tem pode ser parte de outra normalização.
    // Se a fórmula acima sozinha resultar em ganho unitário, você pode precisar remover esta linha.
    // Experimente com e sem ela para ver o que funciona no seu sistema.
    if (overlap_factor_ >= 1.0f) {
