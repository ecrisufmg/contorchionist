...
std::vector<torch::Tensor> SpectralTrailsProcessor<T>::process_frame(
    ...
    // 3. Create reinforcement mask (simply bins above threshold)
    // We no longer require the input magnitude to exceed the stored memory;
    // once a bin is active, we continuously track it with the attack filter.
    torch::Tensor reinforce_mask = above_threshold;

    // Apply decay ONLY to bins that are below threshold.
    memory_magnitude_ = torch::where(reinforce_mask, memory_magnitude_, memory_magnitude_ * decay_);
    ...
    // Update reinforced bins with attack coefficient
    ...
    torch::Tensor output_magnitude = memory_magnitude_;
    torch::Tensor output_phase = memory_phase_;
