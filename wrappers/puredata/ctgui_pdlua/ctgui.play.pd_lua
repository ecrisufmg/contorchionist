...
local thisobj = pd.Class:new():register("ctgui.play")

-- Default Colors (HSB 0-1 scale)
local C_PLAY = {0.333, 0.6, 0.8} -- Green
...
function thisobj:update_layout()
    ...
    -- Play Button
    self.play_btn = {x = x_start, y = y_center - self.btn_size/2, w = self.btn_size, h = self.btn_size}
    ...
    local btn_bottom = self.play_btn.y + self.play_btn.h
    local slider_y = self.height - self.slider_height -2
    -- btn_bottom - self.slider_height
    ...
    -- self.tclock_rect = {
    --     x = x_start,
    --     y = slider_y - 9, -- Just above slider
    --     w = self.width - x_start - 8, -- Avoid info button
    --     h = 7
    -- }
end
...
function thisobj:paint(g)
    ...
    -- Play Button
    set_col(self.c_play, self.state == "play")
    local p = self.play_btn
    
    -- Use Path if available, else rect
    if Path then
        local path = Path(p.x, p.y)
        path:line_to(p.x + p.w, p.y + p.h/2)
        path:line_to(p.x, p.y + p.h)
        path:close()
        g:fill_path(path)
    else
        -- Fallback to rect if Path is missing
        g:fill_rect(p.x, p.y, p.w, p.h)
        g:set_color(255, 255, 255)
        g:draw_text(">", p.x + 10, p.y + 5, 15, 20)
    end
...
function thisobj:in_1_play()
    self:set_state("play", true)
end
...
function thisobj:in_1_set(atoms)
    local cmd = atoms[1]
    if cmd == "play" or cmd == "start" or cmd == 1 then
        self.state = "play"
        self:repaint()
...
function thisobj:in_2(sel, atoms)
    if self.verbose then
        pd.post(string.format("%s: no method for '%s' at inlet 2", self.object_name or "ctgui.play", sel))
    end
end