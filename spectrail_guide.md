...
- **`attackonsetramp`** (`attackonsetramp <frames>` / `attackonsetramptime <segundos>`): suaviza a transição entre `attack` e `attackonset`. A rampa usa `smoothstep` cúbico (início/fim suaves) e, quando expressa em tempo, aceita segundos completos (ex.: `attackonsetramptime 0.015` = 15 ms). Durante a rampa, o valor antigo é preservado nos primeiros frames e só converge para o alvo ao final (mesma curva aplicada à magnitude **e** à fase), reduzindo degraus sincronizados ao hop.
