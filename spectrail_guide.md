...
- **`attackonsetramp`** (`attackonsetramp <frames>` / `attackonsetramptime <segundos>`): suaviza a transição entre `attack` e `attackonset`. A curva agora usa uma sigmóide (ease-in/out) normalizada, de modo que os primeiros frames sobem com suavidade e o valor final encosta gentilmente, reduzindo estalos mesmo com saltos grandes de magnitude.
