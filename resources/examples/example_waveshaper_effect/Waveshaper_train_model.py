...
class WaveshaperModel(nn.Module):
    ...
    def forward(self, ch0: torch.Tensor, ch1: torch.Tensor, ch2: torch.Tensor, ch3: torch.Tensor) -> torch.Tensor:
        """
        Forward pass (compatível com torch.ts~).
        Espera: áudio como bloco de 64 amostras e parâmetros como um único valor por bloco.
        Aceita formatos flexíveis e normaliza internamente.
        """
        
        if ch0.dim() == 1:
            ch0 = ch0.unsqueeze(0)  # [1, block_size]

        audio = ch0  # [N, 64]
        bs = audio.size(1)  # 64

        # aceita formatos flexíveis para parâmetros
        drive = _param_to_control(ch1, bs)
        tone = _param_to_control(ch2, bs)
        mix = _param_to_control(ch3, bs)

        # Garante que N combine entre áudio e parâmetros
        if drive.size(0) != audio.size(0):
            # Repete parâmetros por batch se necessário (ex.: parâmetro único para N blocos)
            if drive.size(0) == 1:
                drive = drive.expand(audio.size(0), 1)
            else:
                raise RuntimeError("Batch size mismatch between audio and parameters (drive)")
        if tone.size(0) != audio.size(0):
            if tone.size(0) == 1:
                tone = tone.expand(audio.size(0), 1)
            else:
                raise RuntimeError("Batch size mismatch between audio and parameters (tone)")
        if mix.size(0) != audio.size(0):
            if mix.size(0) == 1:
                mix = mix.expand(audio.size(0), 1)
            else:
                raise RuntimeError("Batch size mismatch between audio and parameters (mix)")

        # Concatena: [N, 64] + 3x[N,1] -> [N, 67]
    ...
    # def forward(self, ch0: torch.Tensor, ch1: torch.Tensor, ch2: torch.Tensor, ch3: torch.Tensor) -> torch.Tensor:
    #     return self.waveshaperdsp(ch0, ch1, ch2, ch3)



def apply_tone_control(signal: torch.Tensor, tone: float) -> torch.Tensor:
    """
...
def process_block_target(input_block: np.ndarray, drive: float, tone: float, mix: float) -> np.ndarray:
    """
